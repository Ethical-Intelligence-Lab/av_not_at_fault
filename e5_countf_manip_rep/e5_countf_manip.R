...
d <- read.csv('e5_countf_manip_50_coded.csv')
...
# countf_cat_2: 1 - human could have done more; 2 - no change; 3 - passenger would have been in more danger
d <- subset(d, (d$comp1 == 1 & d$comp_accident == 1 & d$countf_cat_2 != 0))
...
## run t-tests
table(d$countf_cat_2)[1]/sum(table(d$countf_cat_2))
table(d$countf_cat_2)[2]/sum(table(d$countf_cat_2))
table(d$countf_cat_2)[3]/sum(table(d$countf_cat_2))
tapply(d$vB_mnfctr_liable_AV_1, d$countf_cat_2, mean)

d <- subset(d, d$countf_cat_2 != 3)

## (1) SUE VEHICLE A DRIVER
vA_liable_T <- t.test(vA_liable_AV_1 ~ countf_cat_2, data = d, paired = FALSE) 
vA_liable_T

## (2) SUE VEHICLE B MANUFACTURER
vB_mnfctr_liable_T <- t.test(vB_mnfctr_liable_AV_1 ~ countf_cat_2, data = d, paired = FALSE) 
vB_mnfctr_liable_T

## (3) VEHICLE B DEFECTIVE
avoid_T <- t.test(avoid_AV_1 ~ countf_cat_2, data = d, paired = FALSE) 
avoid_T
...
# SINGLE MEDIATION
process(data = d, y = "vB_mnfctr_liable_AV_1", x = "countf_cat_2",
