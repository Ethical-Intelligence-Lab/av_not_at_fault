...
d <- read.csv('e1_framework_900.csv')
...
for(i in 1:n_final_AV) {
  ...
  d_subset[i,11] <- as.numeric(d_AV[i,37])
  d_subset[i,1] <- "av"
}
...
for(i in 1:n_final_HDV) {
  ...
  d_subset[j,11] <- as.numeric(d_HDV[i,37])
  d_subset[j,1] <- "human"
}
...
summary(moderate.lm(cond_n, age, vB_m_v_m_liable, d_merged))
